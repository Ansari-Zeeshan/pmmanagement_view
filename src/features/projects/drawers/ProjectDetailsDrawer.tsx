...
export const ProjectDetailsDrawer = ({ task, onClose, onSave, defaultTab = 'UPDATES' }) => {
  ...
  const renderFormattedText = (rawText) => {
    ...
    return lines.map((line, lineIdx) => {
      ...
      parts = parts.flatMap((part) => {
        ...
        return subParts.map((sub, i) => {
          if (sub.startsWith('`') && sub.endsWith('`') && sub.length > 2) {
            return <code key={`c-${i}`} className="bg-light px-1 py-0.5 rounded text-danger" style={{ fontSize: '12px' }}>{sub.slice(1, -1)}</code>;
          }
          return sub;
        });
      });
  ...
  const handleDraftFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newAttached = fileList.map((f: File) => ({
      id: Date.now() + Math.random(),
      name: f.name,
  ...
  const handleFilesDrop = (e) => {
    ...
    droppedFiles.forEach((f: File) => addActivityLog('File Upload', `Uploaded ${f.name} via drag & drop`));
  };
  ...
  const handleComposerDrop = (e: React.DragEvent) => {
    ...
    const newAttached = droppedFiles.map((f: File) => ({
      id: Date.now() + Math.random(),
      name: f.name,
      size: `${(f.size / (1024 * 1024)).toFixed(1)} MB`,
    }));
                  ...
                  <div key={up.id} className="p-3 bg-white rounded-3 border shadow-sm position-relative">
                    ...
                    <div className="d-flex align-items-center justify-content-between mb-2">
                      ...
                      <div className="d-flex align-items-center gap-3 text-muted small">
                        ...
                        <span title="Mute notifications"><Bell size={15} className="cursor-pointer text-muted" /></span>
                      </div>
                    </div>
                    ...
                    <div className="ps-4 ms-2 border-start py-1 mb-2">
                      ...
                      {(up as any).gif && (
                        <div className="mt-2">
                          <img src={(up as any).gif} alt="Attached GIF" className="rounded border" style={{ maxHeight: '180px' }} />
                        </div>
                      )}
                      {(up as any).attachedFiles?.map((f: any, fIdx: number) => (
                      ...
                      <div className="d-flex align-items-start justify-content-between">
                        <div className="d-flex align-items-start gap-3">
                          <img
                            ...
                            onError={(e) => { (e.target as HTMLImageElement).src = '/icons/Files.svg'; }}
                          />
                          <div>
