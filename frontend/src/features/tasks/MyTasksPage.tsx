              ...
              {showAssigneeMenu && (
                <div className="dropdown-menu show shadow-xl p-2 border position-absolute end-0 mt-1 rounded-3 bg-white" style={{ zIndex: 100, minWidth: '190px', maxHeight: '240px', overflowY: 'auto' }}>
                  ...
                  {uniqueAssignees.map((assignee: any) => (
                    <button key={String(assignee)} type="button" className={`dropdown-item rounded-2 py-2 px-3 fw-medium text-truncate ${assigneeFilter === assignee ? 'text-primary bg-primary bg-opacity-10' : 'text-dark'}`} onClick={() => { setAssigneeFilter(assignee); setShowAssigneeMenu(false); }}>
                      {String(assignee)}
                    </button>
                  ))}
                </div>
