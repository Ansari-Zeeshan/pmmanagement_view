...
export const parseDateRangeToISO = (plannedDateStr, index = 0) => {
  if (plannedDateStr) {
    const parts = plannedDateStr.split(/\s*-\s*|\s*to\s*/i);
    if (parts.length === 2) {
      ...
      const parsePart = (str, isEnd = false) => {
        ...
        if (match) {
          ...
          const mIdx = monthMap[mStr] !== undefined ? monthMap[mStr] : 10;
          const day = String(parseInt(match[2], 10)).padStart(2, '0');
          let yr = match[3] ? parseInt(match[3], 10) : 2026;
          if (isEnd && mIdx < 6 && !match[3]) yr = 2027;
          const mFormatted = String(mIdx + 1).padStart(2, '0');
          return `${yr}-${mFormatted}-${day}`;
        }
        return null;
      };

      const startISO = parsePart(p1, false);
      const endISO = parsePart(p2, true);
      if (startISO && endISO) {
  ...
  const baseMonth = (9 + (index % 4)) % 12;
  const startYr = 2026;
  const endYr = baseMonth >= 10 ? 2027 : 2026;
  const sMonthStr = String(baseMonth + 1).padStart(2, '0');
  const eMonthStr = String(((baseMonth + 2) % 12) + 1).padStart(2, '0');
  return {
    startDate: `${startYr}-${sMonthStr}-01`,
    endDate: `${endYr}-${eMonthStr}-28`
  };
};
...
export const GanttView = ({ tasks = [], onTaskClick }) => {
  ...
  // Dynamically map projects from store database into 3-tier Gantt tree structure
  const allProjectsRaw = useMemo(() => {
    const dataset = Array.isArray(tasks) && tasks.length > 0 ? tasks : INITIAL_PROJECTS;

    return dataset.map((t, idx) => {
      const dates = parseDateRangeToISO(t.plannedDate, idx);
      const startDate = t.startDate || dates.startDate;
      const endDate = t.endDate || dates.endDate;
      const range = formatGanttBarDateRange(startDate, endDate);

      // Color theme based on project group
      let barColor = '#d97706'; // Research (Amber)
      if (t.group === 'Wireframe') barColor = '#2563eb'; // Royal Blue
      else if (t.group === 'Visual Studio') barColor = '#7c3aed'; // Purple
      else if (t.group === 'Development') barColor = '#059669'; // Emerald

      const criticalPath = t.priority === 'Critical' || t.priority === 'High' || idx % 2 === 0;

      // Milestones mapping
      let milestones = [];
      if (t.milestones && Array.isArray(t.milestones) && t.milestones.length > 0) {
        milestones = t.milestones.map((m, mIdx) => {
          const mDates = m.date || m.dueDate ? parseDateRangeToISO(m.date || m.dueDate, mIdx) : { startDate, endDate };
          const mStart = m.startDate || mDates.startDate;
          const mEnd = m.endDate || mDates.endDate;

          const subtasks = (m.tasks && Array.isArray(m.tasks) && m.tasks.length > 0)
            ? m.tasks.map((st, stIdx) => {
                const stDates = st.date ? parseDateRangeToISO(st.date, stIdx) : { startDate: mStart, endDate: mEnd };
                return {
                  id: st.id || `st-${idx}-${mIdx}-${stIdx}`,
                  code: `TSK-${201 + stIdx}`,
                  title: st.title || `Subtask ${stIdx + 1}`,
                  type: 'TASK',
                  startDate: stDates.startDate,
                  endDate: stDates.endDate,
                  range: formatGanttBarDateRange(stDates.startDate, stDates.endDate),
                  duration: `${st.duration || '20 Days'}`,
                  percentage: `${st.status === 'Completed' || st.status === 'Approved' ? 100 : st.status === 'In Progress' ? 50 : 0}%`,
                  status: st.status || 'Pending',
                  priority: st.priority || 'Medium',
                  plannedBudget: 'AED 30,000',
                  actualBudget: 'AED 20,000',
                  barColor: '#b45309',
                  criticalPath: stIdx === 0,
                  assignee: { name: st.assignee || t.owner || 'Claire Bure', role: 'Assignee', avatar: '/img/client1.jpg' },
                  rawTask: t,
                };
              })
            : [
                {
                  id: `st-${idx}-${mIdx}-default`,
                  code: `TSK-${301 + mIdx}`,
                  title: `${m.title} - Scope Execution & Delivery`,
                  type: 'TASK',
                  startDate: mStart,
                  endDate: mEnd,
                  range: formatGanttBarDateRange(mStart, mEnd),
                  duration: '15 Days',
                  percentage: `${m.percentageDone !== undefined ? m.percentageDone : 50}%`,
                  status: m.status || 'In Progress',
                  priority: 'Medium',
                  plannedBudget: 'AED 40,000',
                  actualBudget: 'AED 25,000',
                  barColor: '#b45309',
                  criticalPath: mIdx === 0,
                  assignee: { name: m.owner || t.owner || 'Claire Bure', role: 'Assignee', avatar: '/img/client1.jpg' },
                  rawTask: t,
                },
              ];
          ...
          return {
            ...
            percentage: `${m.percentageDone !== undefined ? m.percentageDone : m.status === 'Completed' || m.status === 'Approved' ? 100 : 50}%`,
            status: m.status || 'In Progress',
            plannedBudget: m.plannedBudget || 'AED 60,000',
            actualBudget: m.actualBudget || 'AED 40,000',
            barColor: '#f59e0b',
            criticalPath: mIdx === 0,
            lead: { name: m.owner || t.owner || 'Claire Bure', role: 'Milestone Lead', avatar: '/img/client1.jpg' },
            tasks: subtasks,
            rawTask: t,
          };
        });
      } else {
        // Default structure if project has no milestones array
        milestones = [
          {
            id: `m-${idx}-1`,
            code: `MS-${101 + idx}`,
            title: `${t.title} - Phase 1: Planning & Specification`,
            type: 'MILESTONE',
            startDate: startDate,
            endDate: endDate,
            range: range,
            duration: '30 Days',
            percentage: `${t.progress || 50}%`,
            status: t.status || 'In Progress',
            plannedBudget: t.plannedBudget || 'AED 100,000',
            actualBudget: t.actualBudget || 'AED 60,000',
            barColor: '#f59e0b',
            criticalPath: true,
            lead: { name: t.owner || t.customer || 'Claire Bure', role: 'Milestone Lead', avatar: '/img/client1.jpg' },
            tasks: [
              {
                id: `t-${idx}-1`,
                code: `TSK-${201 + idx}`,
                title: `${t.title} - Scope Definition & Approval`,
                type: 'TASK',
                startDate: startDate,
                endDate: endDate,
                range: range,
                ...
                plannedBudget: t.plannedBudget || 'AED 50,000',
                actualBudget: t.actualBudget || 'AED 30,000',
                barColor: '#b45309',
                criticalPath: true,
                assignee: { name: t.owner || 'Ajmal Khan', role: 'Assignee', avatar: '/img/client2.jpg' },
                rawTask: t,
              },
            ],
            rawTask: t,
      ...
      return {
        id: t._id || t.id || `proj-${idx}`,
        reference: t.reference || `PRJ-${1024 + idx}`,
        title: t.title,
        type: 'PROJECT',
        ...
        endDate: endDate,
        range: range,
        duration: t.totalDays || '60 Days',
        percentage: `${t.progress || 0}%`,
        status: t.status || 'On Track',
        plannedBudget: t.plannedBudget || 'AED 200,000',
        actualBudget: t.actualBudget || 'AED 150,000',
        barColor: barColor,
        criticalPath: criticalPath,
        domainLead: {
          name: t.owner || t.customer || t.domainLead || 'Claire Bure',
          role: 'Project Lead',
          avatar: t.assignees?.[0]?.avatarUrl || '/img/client1.jpg',
        },
          ...
          <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
            ...
            {selectedScale === 'Days' && (
              <FormControl size="small" sx={{ width: 115 }}>
                <Select
                  value={selectedMonth}
                  onChange={(e) => setSelectedMonth(parseInt(String(e.target.value), 10))}
            ...
            <FormControl size="small" sx={{ width: 95 }}>
              <Select
                value={selectedYear}
                onChange={(e) => setSelectedYear(parseInt(String(e.target.value), 10))}
                sx={{ height: 34, fontWeight: 700, color: 'primary.main', fontSize: '11px' }}
              >
                {yearOptions.map((y) => (
              ...
              return (
                <tr key={row.id} style={{ height: '48px' }} className={rowClass}>
                  ...
                  <td className="border-end" style={{ maxWidth: '300px' }}>
                    ...
                    >
                      ...
                      <div className="d-flex align-items-center gap-1 overflow-hidden">
                        ...
                        {row.isMilestone && <Folder size={14} className="text-warning flex-shrink-0" />}
                        {row.isTask && <CheckSquare size={14} className="text-info flex-shrink-0" />}
                        ...
                        {row.criticalPath && (
                          <span title="Critical Path Item"><Star size={12} className="text-warning fill-warning flex-shrink-0" /></span>
                        )}
                      </div>
                    </div>
                  </td>
                  ...
                  <td colSpan={totalCols} className="position-relative p-0" style={{ height: '48px' }}>
                    ...
                    <div className="position-absolute top-0 start-0 w-100 h-100 d-flex pointer-events-none" style={{ zIndex: 1 }}>
                      {Array.from({ length: totalCols }).map((_, colIdx) => (
                        <div
                          key={colIdx}
                          className={`h-100 border-end ${
                            selectedScale === 'Days' && (new Date(selectedYear, selectedMonth, colIdx + 1).getDay() === 0 || new Date(selectedYear, selectedMonth, colIdx + 1).getDay() === 6)
