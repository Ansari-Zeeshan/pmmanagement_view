...
export const MuiPremiumDatePicker: React.FC<MuiPremiumDatePickerProps> = ({
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const isDisabled = Boolean(
      (minDate && dayDateStr < minDate) || (maxDate && dayDateStr > maxDate)
    );
              ...
              <Grid item xs={12 / 7} key={idx} sx={{ textAlign: 'center' }}>
                <Box
                  ...
                  sx={{
                    ...
                    color: isDisabled ? '#94a3b8' : cell.isSelected ? '#ffffff' : cell.isToday ? '#2563eb' : '#1e293b',
                    backgroundColor: cell.isSelected ? '#2563eb' : 'transparent',
                    border: cell.isToday && !cell.isSelected ? '1px solid #2563eb' : 'none',
                    transition: 'all 0.15s ease',
                    '&:hover': {
                      backgroundColor: isDisabled ? 'transparent' : cell.isSelected ? '#1d4ed8' : '#eff6ff',
                      color: isDisabled ? '#94a3b8' : cell.isSelected ? '#ffffff' : '#2563eb',
                    },
                  }}
                >
                  {cell.day}
